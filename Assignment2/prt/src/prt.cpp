...
public:
	...
	virtual void preprocess(const Scene *scene) override
	{
		...
		if (m_Type == Type::Interreflection)
		{
			...
			Eigen::MatrixXf InterReflectCoef;
			InterReflectCoef.setZero(SHCoeffLength, mesh->getVertexCount());
			for (size_t verId = 0; verId < mesh->getVertexCount(); ++verId)
			{
				const Point3f &v = mesh->getVertexPositions().col(verId);
				const Normal3f &n = mesh->getVertexNormals().col(verId);

				Ray3f ray_on(v, -n);
				Intersection inter;
				inter.bary = Vector3f(1.0, 0.0, 0.0);
				inter.mesh = mesh;
				inter.tri_index = Vector3f(verId);

				for (size_t i = 0; i < m_SampleCount; i++)
				{
					InterReflectCoef.col(verId) += getInterReflection(inter, ray_on, 0, scene);
				}

				InterReflectCoef.col(verId) /= float(m_SampleCount);
				if (0 == verId % 500)//FOR DEBUG
				{
					std::cout << "process: " << verId + 1 << "/" << mesh->getVertexCount() << std::endl;
				}
			}
			m_TransportSHCoeffs = InterReflectCoef;
		}
	...
	Eigen::VectorXf getInterReflection(Intersection inter, Ray3f ray, int depth, const Scene*scene)
	{
		if (depth>m_Bounce)
		{
			Eigen::VectorXf res;
			res.setZero(SHCoeffLength);
			return res;
		}
		else
		{
